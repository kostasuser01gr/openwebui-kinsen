...
const ALL_EVENTS: WebhookEvent[] = [
  'chat.message', 'escalation.created', 'escalation.resolved',
  ...
  return (
    <div className="webhooks-tab">
      <div className="webhooks-header">
        <h3>Webhooks</h3>
        <button className="btn-primary btn-small" onClick={() => setShowForm(true)}>+ Add Webhook</button>
      </div>
      ...
      {showForm && (
        <div className="webhook-form">
          ...
          <div className="escalation-form-actions">
            <button className="btn-primary" onClick={createWebhook}>Create Webhook</button>
          ...
          <div key={wh.id} className={`webhook-card ${wh.active ? '' : 'inactive'}`}>
            <div className="webhook-card-header">
              ...
              <label className="toggle-switch">
                <input type="checkbox" checked={wh.active} onChange={e => toggleActive(wh.id, e.target.checked)} />
                <span className="toggle-slider" />
              </label>
            </div>
            <div className="webhook-events">
              {wh.events.map(e => <span key={e} className="webhook-event-tag">{e}</span>)}
            </div>
            <div className="webhook-meta">
              ...
              {wh.failCount > 0 && <span className="webhook-fails">⚠️ {wh.failCount} failures</span>}
              <button className="btn-small btn-danger" onClick={() => deleteWebhook(wh.id)}>Delete</button>
            </div>
          </div>
        ))}
      </div>
