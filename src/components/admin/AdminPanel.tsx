...
export function AdminPanel({ user, darkMode, onToggleDark, onBack }: Props) {
  ...
  const tabs: { id: Tab; label: string; icon: string; minRole: string[] }[] = [
    { id: 'knowledge', label: 'Knowledge Base', icon: '📚', minRole: ['supervisor', 'manager', 'admin'] },
    { id: 'analytics', label: 'Analytics', icon: '📊', minRole: ['supervisor', 'manager', 'admin'] },
    { id: 'users', label: 'Users', icon: '👥', minRole: ['manager', 'admin'] },
    { id: 'sessions', label: 'Sessions', icon: '🔐', minRole: ['agent', 'supervisor', 'manager', 'admin'] },
    ...
    <div className="admin-container">
      <header className="admin-header">
        <div className="header-left">
          <button className="icon-btn" onClick={onBack} title="Back to Chat">← Chat</button>
          ...
          <div>
            <h1>Admin Panel</h1>
            <span className="header-tag">{user.name} · {user.role}</span>
          </div>
        </div>
