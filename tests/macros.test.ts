...
describe('executeMacro', () => {
  ...
  it('calculates fuel charge for 20 missing litres', () => {
    ...
    const result = executeMacro(macro, { missing_litres: '20', is_electric: 'No', missing_kwh: '0' });
    expect(result).toContain('€75.00');
  });

  it('calculates EV charge fee', () => {
    const macro = MACROS.find(m => m.id === 'fuel-charge-calc')!;
    const result = executeMacro(macro, { missing_litres: '0', is_electric: 'Yes', missing_kwh: '30' });
  ...
  it('calculates mileage overage for economy', () => {
    ...
    const result = executeMacro(macro, { km_driven: '1500', km_included: '1000', vehicle_class: 'Economy' });
  ...
  it('shows no overage when under limit', () => {
    const macro = MACROS.find(m => m.id === 'mileage-overage-calc')!;
    const result = executeMacro(macro, { km_driven: '800', km_included: '1000', vehicle_class: 'Midsize' });
  ...
  it('calculates free cancellation for 72h notice', () => {
    ...
    const result = executeMacro(macro, {
      hours_before: '72', daily_rate: '60', booking_type: 'Pay at Counter', total_prepaid: '0',
    });
