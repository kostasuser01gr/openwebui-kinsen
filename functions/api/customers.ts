  ...
  if (type === 'customer' || type === 'all') {
    ...
    for (const cid of custIds) {
      ...
      if (raw) {
        ...
        if (!q || cust.name.toLowerCase().includes(q) || cust.email.toLowerCase().includes(q) ||
            cust.phone.includes(q) || cust.id.toLowerCase().includes(q) ||
  ...
  if (type === 'booking' || type === 'all') {
    ...
    for (const bid of bkIds) {
      ...
      if (raw) {
        ...
        if (!q || bk.id.toLowerCase().includes(q) || bk.vehiclePlate.toLowerCase().includes(q) ||
            bk.customerId.toLowerCase().includes(q) || bk.vehicleClass.toLowerCase().includes(q)) {
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  return new Response(JSON.stringify({ ok: true, customers: custIds.length, bookings: bkIds.length }));
};
